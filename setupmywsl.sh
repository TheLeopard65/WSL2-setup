...
sudo apt update -y
sudo apt upgrade -y
sudo apt install -y burpsuite ghidra wireshark nmap netdiscover exiftool tor torbrowser-launcher dpkg john network-manager eog aircrack-ng steghide scapy hashcat python3.13 theharvester hping3 gobuster dirbuster autopsy masscan maltego chisel foremost pdf-parser  ncat reaver wifite netcat-traditional sqlmap zbar-tools metasploit-framework iptables python3-pip nikto  checksec ffmpeg plocate hydra binwalk git python3-pip python3-dev libssl-dev libffi-dev build-essential libwine winericks
